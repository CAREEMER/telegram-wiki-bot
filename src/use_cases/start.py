...
from services.language import get_language, get_primary_language, list_languages
...
async def show_start_message(user: User, session: AsyncSession):
    ...
    user_language = await get_language(user.language_id, session)

    reply_markup = types.InlineKeyboardMarkup(
        inline_keyboard=[
            [
                types.InlineKeyboardButton(
                    text=Texts.get(Text.LIST_ARTICLES_BUTTON, user_language.code), callback_data="list_articles:0:1"
                )
            ]
        ]
    )
    ...
    if user.is_redactor:
        reply_markup.inline_keyboard.append(
            [
                types.InlineKeyboardButton(
                    text=Texts.get(Text.CREATE_ARTICLE_BUTTON, user_language.code), callback_data="create_article:"
                )
            ]
        )
    ...
    message_text = Texts.get(Text.START_MESSAGE, user_language.code) + "\n\n"
    ...
    for main_article in main_articles:
        ...
        message_text += f"[{title.content}]({bot_link}?start=article_{main_article.id})\n\n"
